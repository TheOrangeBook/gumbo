...
assemblyinfo :global_version do |asm|
  ...
  asm.description = "gumbo, dynamic objects"
...
task :compile => [:versioning, :global_version, :build] do
	copyOutputFiles File.join(props[:src], "gumbo/bin/Release"), "gumbo.{dll,pdb,xml}", File.join(props[:output], 'net-4.0')
end
...
nunit :tests => [:compile] do |nunit|
          ...
          nunit.assemblies = FileList[File.join(props[:src], "gumbo.specs/bin/Release", "gumbo.Tests.dll")]
end
...
zip :zip_output => [:versioning] do |zip|
	...
	zip.output_file = "gumbo-#{NUGET_VERSION}.zip"
	zip.output_path = props[:artifacts]
end

desc "Restore NuGet Packages"
task :nuget_restore do
  sh "lib/nuget install #{File.join(props[:src],"gumbo.specs","packages.config")} -o #{File.join(props[:src],"packages")}"
end

desc "Builds the nuget package"
task :nuget => [:versioning, :create_nuspec] do
	sh "lib/nuget pack #{props[:artifacts]}/gumbo.nuspec /Symbols /OutputDirectory #{props[:artifacts]}"
end
...
nuspec :_nuspec do |nuspec|
  ...
  nuspec.description = 'gumbo, dynamic objects for .NET Applications'
  nuspec.title = 'gumbo'
  nuspec.projectUrl = 'http://github.com/TheOrangeBook/gumbo'
  nuspec.language = "en-US"
  nuspec.licenseUrl = "http://www.apache.org/licenses/LICENSE-2.0"
  nuspec.requireLicenseAcceptance = "false"
  nuspec.output_file = File.join(props[:artifacts], 'gumbo.nuspec')
  add_files props[:output], 'gumbo.{dll,pdb,xml}', nuspec
  nuspec.file(File.join(props[:src], "gumbo\\**\\*.cs").gsub("/","\\"), "src")
end
